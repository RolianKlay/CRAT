...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, topk, window_size=(7, 7, 7), shift_size=(0, 0, 0),
        ...
        # Default Biformer Block
        # self.qk_dim = dim
        # self.kv_per_win = 4
        # self.kv_downsample_ratio = 4
        # self.kv_downsample_kernel = None
        # self.kv_downsample_mode = 'ada_avgpool'
        # self.topk = 1
        # self.param_attention = "qkvo"
        # self.param_routing = False
        # self.diff_routing = False
        # self.soft_routing = False
        # self.side_dwconv = 5
        # self.auto_pad = False

        self.attn = BiLevelRoutingAttention(topk=topk, dim=dim, num_heads=num_heads, n_win=window_size[0])
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    #
...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x: B, H*W, C
        """
        D, H, W = self.input_resolution # (4, 4, 4) x: (1, 4, 576, 32, 4) ori_x: (1, 576, 32, 4, 4)
        x = x.flatten(2).transpose(1, 2) # (1, 73728, 4) ori_x: (1, 512, 576)
        x = self.expand(x) # error
        B, L, C = x.shape
        # assert L == D * H * W, "input feature has wrong size"
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, topk, window_size=(7, 7, 7),
...
class SwinTransformerSys3D(nn.Module):
    ...
    def forward_features(self, x):
        x = self.patch_embed(x) # (1, 4, 128, 128, 128 ) -> (1, 72, 32, 32, 32)
        x = self.pos_drop(x) #
        x_downsample = []
        # v_values_1 = []
        # k_values_1 = []
        ...
        # x = rearrange(x, 'n c d h w -> n d h w c') # x: (1, 576, 32, 4, 4)
        # x = self.norm(x)
