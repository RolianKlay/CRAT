...
parser = argparse.ArgumentParser(description='CRAT BRATS 2021 Training')
...
parser.add_argument('--topk', default=4, type=int, help='The topk related conections')
...
def main(args):
    ...
    model_1 = ViT_seg(config, topk=args.topk, num_classes=args.num_classes, 
    ...
    # Check FLOPs - fvcore
    test_tensor = (torch.rand(1, 4, 128, 128, 128)).cuda()
    flops_fvcore = FlopCountAnalysis(model_1, test_tensor)
    print("FLOPs_fvcore: ", flops_fvcore.total())
    print(parameter_count_table(model_1))
    exit()

    #  # test_tensor = (torch.rand(4, 128, 128, 128)).cuda()
    # flops, params = get_model_complexity_info(model_1, (4, 128, 128, 128), as_strings=True, print_per_layer_stat=True)
    # print("%s |%s |%s" % ("CRAT", flops, params))
    # exit()
